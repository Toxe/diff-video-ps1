...
function ExtractFrames {
    ...
    WithDuration 'extracting frames...' {
        ...
        $min_frames = [math]::Min($video1_number_of_frames, $video2_number_of_frames)
        $max_frames = [math]::Max($video1_number_of_frames, $video2_number_of_frames)

        if ($min_frames -ne $max_frames) {
            # the videos have different numbers of frames, so remove the excess frames
            ...
            for ($i = $min_frames + 1; $i -le $max_frames; ++$i) {
